...
- Enrutamiento Global: Permite dirigir a los usuarios a la región más cercana o de mejor rendimiento
...
| **Aspecto**                | **AWS Global Accelerator**                                                             | **AWS Route 53**                                                        |
|----------------------------|----------------------------------------------------------------------------------------|--------------------------------------------------------------------------|
| **Propósito Principal**    | Mejorar la **disponibilidad** y **baja latencia** de aplicaciones globales.            | Proporcionar **DNS gestionado** para dominios con funciones avanzadas.   |
| **Enrutamiento**           | Basado en latencia y salud de los endpoints para aplicaciones.                         | Basado en políticas DNS (geográficas, ponderadas, de failover, etc.).    |
| **Protocolo Soportado**    | TCP y UDP (puede enrutar tráfico directamente al backend).                             | DNS (resuelve nombres de dominio en direcciones IP).                     |
| **Configuración**          | Asocia endpoints (ALB, NLB, IP estáticas, instancias EC2, etc.).                      | Configura registros DNS (A, CNAME, MX, etc.) para dominios personalizados.|
| **IP Estática**            | Proporciona direcciones IP estáticas globales reutilizables.                          | No proporciona IP estáticas.                                            |
| **Casos de Uso**    | - APIs globales para aplicaciones distribuidas.<br>- Streaming de medios.<br>- Juegos en línea.<br>- Aplicaciones con requisitos de latencia mínima. | - Distribución de tráfico web para dominios personalizados.<br>- Hosting de múltiples servicios bajo un mismo dominio. |
...
**Video**: [Video](https://ulima-edu-pe.zoom.us/rec/share/6TZ-My_uIxPBVMhrJQ2L_63vFSJmGB5SVuoCB59b4ZDKh_vdQWgP8oAwyzLLK7bm.nCDF8OZ9e4hrObAZ)
