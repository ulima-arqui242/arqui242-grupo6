...
![Crear_records](create_records.png)

Se utiliza el subdominio "api" y se crea el registro A – IPv4 address. Luego se enciende el alias, se selecciona que consiste en un API Gateway, se escoge su región y se pega la URL correspondiente del API REST desplegado. Luego se elige Latency como Routing policy y se reitera la región a la que estará asociada dicha API. Por ultimo, se enciende el Evaluate target health (si no aparece en la pantalla de creacion, luego de crearlo es posible editarlo para encenderlo).

Finalmente, lo que se debería realizar es adquirir un dominio DNS mediante Route53 o otros proveedores externos y luego otorgar los permisos de gestion a Route53 y ya se podría vincular el DNS el cual enrutaría a la region de Sao Paulo o Ohio dependieendo de la latencia.

Sin embargo, como no se cuenta con el presupuesto para la compra de un dominio, se realiza las pruebas de cada una de las regiones mediante la URL proveida por sus respectivos API Gateways.
