...
## LLMs usualmente utilizados para generación de código: 
GPT-4
Codex
Utilizados para:
- Generación de código: Crear fragmentos de código a partir de descripciones en lenguaje natural.
- Depuración y optimización: Identificar y corregir errores o sugerir mejoras en el código existente.
- Documentación: Generar comentarios o explicaciones para mejorar la comprensión del código.
- Traducción de código: Convertir código entre diferentes lenguajes de programación.
- Aprendizaje y tutorías: Responder preguntas técnicas y enseñar conceptos de programación.

## Como se crean estos modelos?

- Entrenamiento en datos masivos: Aprenden patrones y sintaxis de grandes volúmenes de código fuente y documentación. (Ejemplo: Llama4)
- Comprensión del contexto: Analizan el texto ingresado para generar respuestas o sugerencias relevantes.
**Modelos específicos como Codex:**
- Derivado de GPT, entrenado específicamente con repositorios públicos de código.
- Especializado en lenguajes de programación y tareas de desarrollo.


## Modelos utilizados
Para este caso se utiliza la plataforma **Hugging faces**, una plataforma líder en aprendizaje automático y procesamiento de lenguaje natural, la cual ofrece una biblioteca de herramientas para implementar y entrenar modelos de lenguaje grandes (LLMs) de manera accesible.

Para este caso se utiliza la plataforma **Hugging faces**, una plataforma líder en aprendizaje automático y procesamiento de lenguaje natural, la cual ofrece una biblioteca de herramientas para implementar y entrenar modelos de lenguaje grandes (LLMs) de manera accesible.

- GPT-Neo: Modelo de lenguaje de código abierto similar a GPT-3, diseñado para tareas generales de generación de texto.
- CodeParrot: Modelo especializado en generación de código, entrenado en grandes conjuntos de datos de repositorios públicos.
- GPT-4: Última versión avanzada de la serie GPT de OpenAI, enfocada en comprensión y generación de texto de alta calidad.



Para realizar un benchmarking efectivo, la plataforma especifica utilizar librerías como `transformers` de Hugging Face para cargar modelos, junto con herramientas como `datasets` para evaluar su rendimiento en conjuntos de datos relevantes. 

## Ejemplo de configuración e importación de librerías

```python
# Instalar las librerías necesarias
# pip install transformers datasets evaluate

# Importar las librerías para el benchmarking
...
# Cargar un conjunto de datos de evaluación
dataset = load_dataset("code_x_glue_ct_code_to_text", "java")

# Evaluación del modelo con métricas predefinidas
metric = evaluate.load("accuracy")
predictions = []
references = []


#ejemplo para generar el codigo
...
# Calcular metricas como precision
accuracy = metric.compute(predictions=predictions, references=references)
print("Accuracy del modelo:", accuracy)
```

El codigo mostrado demuestra un codigo general que es utilizado con la libreria de Hugging Faces para generar codigo con modelos ya cargados (codigo extraido de su pagina)

Para todos los modelos , se utiliza el mismo prompt, el cual sera en este caso: **"Write a python function that sorts an array of strings alphabetically"**

## Modelo GPT-Neo

Para el modelo GPT-Neo, se utiliza el siguiente codigo que incluye el prompt y su metrica como tiempo de ejecucion
```
import torch
import time
...
Al correr el codigo, conseguimos los siguientes resultados: 
![image](https://github.com/user-attachments/assets/a2e44c48-2533-4845-9f1e-3b585ea317f0)


![image](https://github.com/user-attachments/assets/05f58f51-94f1-4327-861f-ee06a668d9da)

El codigo se ejecuta en aproximadamente minuto y medio, resultando en una funcion de python que, a simple vista, parece que funcionaria, pero para ponerlo a prueba se necesita ejecutarlo.

Para ejecutar y probar el codigo generado por el modelo, se usa el siguiente codigo: 
```
...
![image](https://github.com/user-attachments/assets/4915aa81-940d-4037-9362-785e51ca1982)




- Para el modelo GPT-Neo, se puede verificar que funciona el codigo generado.
- Tiempo de ejecucion relativamente largo

Algo a considerar con este modelo, es su necesidad de especificar mucho la funcion de codigo, su funcion de autollenado no es muy buena, pues se intento utilizar un prompt menos especifico consiguiendo la siguiente funcion y resultado.


![image](https://github.com/user-attachments/assets/fd281811-6111-42ad-81ce-d141622dbb8d)

El modelo genero una funcion que no hace un sort alfabeticamente, sino un sort con indices. Solo luego de ajustar el prompt se consiguieron los resultados iniciales.

# Modelo Code Parrot
Este modelo se puede cargar mediante el siguiente codigo: 
```
from transformers import AutoModelForCausalLM, AutoTokenizer
...
print(generated_code)
```
Siguiendo estos pasos, se carga el modelo code Parrot mediante Hugging Faces, al correr el modelo, se consigue el siguiente resultado:



![image](https://github.com/user-attachments/assets/d920a3a6-0de3-4148-aa10-53f2ac07257c)


En este caso, el modelo genera una funcion que no se puede ejecutar en un entorno como Visual Studio, pues es una funcion incompleta y luego de generar una linea, se repite la misma linea una "N" cantidad de veces. Esto se puede deber a la naturaleza del modelo, una falta de entrenamiento del modelo conseguido, o la posibilidad que los datos con los que esta entrenado no son los mejores para atacar problemas de generacion de codigo, sin embargo, CodeParrot deberia generar una funcion sin problema, pues es un modelo para generacion de codigo.

En este caso , no se puedo probar su ejecucion debido a que el codigo generado no podria correr.

# Modelo GPT-4
Uno de los modelos mas utilizados en el dia a dia es GPT-4, la ultima version de OpenAI, siendo el modelo mas ambicioso y entrenado de la compania, en este caso , se utiliza el mismo prompt para generar un codigo. Para esta generacion de codigo, se utiliza la plataforma de OpenAI para darle un prompt y que genere codigo.

![image](https://github.com/user-attachments/assets/a8e74056-9b04-46cf-85a8-ee433bda99ff)

La funcion generado por el modelo GPT-4 fue el siguiente:
```
def sort_strings_alphabetically(strings):
    """
    Sort an array of strings alphabetically.
...
```
El codigo generado respeta la indexacion, el lenguaje, comprende rapidamente sin necesidad de ajustar el prompt. En este caso, el tiempo de ejecucion fue un tiempo impresionante con solo 3 segundos. 

Al momento de probar el codigo, se consigue lo siguiente: 

![image](https://github.com/user-attachments/assets/d1d55582-baab-49e4-bd38-cc200ad9388d)

Para el modelo GPT-4, la ejecucion fue exitosa y se puede corroborar un funcionamiento correcto de la funcion generada, pues al alimentarle el mismo tipo de array de strings, la funcion y codigo hace un sort alfabetico, tal como dice el prompt.


# Comparacion entre modelos:
Para un mejor entendimiento de los modelos y las funciones que generaron, se pueden realizar conclusiones y comparaciones directas entre modelos.

**Calidad del Código Generado:**
- GPT-Neo: Inicialmente generó un código incorrecto hasta que se ajustó el prompt para ser más específico. Posteriormente, produjo un código correcto y funcional.
- CodeParrot: Aunque generó código más rápido, este fue incompleto y no utilizable para pruebas, lo que limita su efectividad en tareas prácticas.
- 
**Comparación:** Modelos más avanzados como GPT-4 o LLaMA 2, que han sido entrenados en conjuntos de datos más grandes y diversos, tienden a generar código funcional y utilizable en el primer intento con prompts generales.


## Comparacion en diferentes aspectos de los modelos:

| Modelo     | Tiempo de Ejecución        | Calidad del Código                       | Facilidad de Uso                         | Complejidad Soportada   | Costo |
|------------|----------------------------|------------------------------------------|------------------------------------------|-------------------------|-------|
| GPTNeo     | 90 segundos de promedio    | Funcional después de ajustar el prompt  | Requiere iteraciones y ajustes en prompts | Tareas de mediana complejidad | Bajo  |
| CodeParrot | 60 segundos promedio       | Incompleto y no ejecutable              | Resultados poco útiles                   | Baja                    | Bajo  |
| GPT-4      | Menos de 10 segundos       | Funcional y utilizable desde primer intento | Excelente interpretación de prompts     | Alta                    | Alto  |



Cabe recalcar que por motivos de problemas de dependencias, costo adicional de API-keys, el modelo GPT-4 no fue cargado en un ambiente local, una razon por la que haya tenido una ventaja adicional a los modelos cargados de manera local. Para una futura comparacion a mas detalle, todos los modelos deberian cargarse bajo las mismas condiciones.


**Exposicion:** [Link del video](https://drive.google.com/file/d/1mv-LOaBqzT5jN8PPVr17_pY8zNrQXbd6/view?usp=sharing)
