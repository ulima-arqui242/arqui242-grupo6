...
Este ejemplo implementa un sistema de *sharding* con particionado por rango usando `user_id` como clave de partición. Usaremos **ProxySQL** como middleware para distribuir las consultas entre tres servidores MySQL.
...
2. **Configurar las reglas de redirección de consultas a los shards correspondientes:**
   ...
   - Aquí, creamos reglas que dirigen las consultas basadas en el user_id a los shards correspondientes:
   ```sql
...
3. **Aplicar y guardar la configuración:**
   ...
   - Una vez que se agregan los servidores y las reglas, se carga y guarda la configuración para que ProxySQL la use en tiempo real:
   ```sql
...
### **3. Middleware en Node.js para Consultar Datos** ###
  ...
  2. Código del Middleware en Node.js:
     ```javascript
      ...
      // Configura la conexión a ProxySQL
