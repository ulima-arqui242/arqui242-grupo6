...
| ----------------------- | ----------------------- | -------------------------- | ------------------------------------------------------------------------------------------ | ----------------------------- | -------------------------------------------------- | ------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------- |
| ESC-01            | Disponibilidad          | Pico de tráfico inesperado | Gran cantidad de usuarios intenta acceder simultáneamente al módulo de gestión de usuarios | Módulo de Gestión de Usuarios | Operación en horas pico durante un evento especial | El sistema debe adaptarse al aumento de tráfico y continuar funcionando sin interrupciones | El sistema debe atender al menos el 95% de las solicitudes durante el pico de tráfico |
...
| ----------------- | ----------------------- | --------------------------------------- | ----------------------- | ----------------- | ------------------------------------------------ | -------------------------------------------------------------------------------- | ----------------------------------------------------------------------- |
| ESC-02            | Disponibilidad          | Fallo en la conexión a la base de datos | Servidor de la nube     | Módulo de Hábitos | Operación normal durante el uso de la aplicación | El sistema sigue funcionando, los datos se sincronizan cuando vuelve la conexión | El sistema puede operar offline durante 10 minutos sin pérdida de datos |
...
| **Cod Escenario** | **Atributo de Calidad** | **Estímulo**                            | **Fuente del Estímulo** | **Artefacto**       | **Entorno**           | **Respuesta**                                    | **Medida de Respuesta**                                                                  |
| ----------------- | ----------------------- | --------------------------------------- | ----------------------- | ------------------- | --------------------- | ------------------------------------------------ | ---------------------------------------------------------------------------------------- |
| ESC-03            | Disponibilidad          | Fallo en la conexión a la base de datos | Servidor de la nube     | Módulo de Seguimiento | Operación normal durante el uso de la aplicación | El sistema sigue funcionando y los datos se sincronizan cuando se restablece la conexión | El sistema puede operar offline durante 15 minutos sin pérdida de datos |El sistema puede operar offline durante 15 minutos sin pérdida de datos |

### Tactica elegida

Para el módulo de Seguimiento, se ha seleccionado la táctica de Reintentos, que permite mantener alta disponibilidad sin una inversión considerable en infraestructura adicional. Con esta táctica, se asegura que el módulo de Seguimiento continúe operativo incluso si ocurre una falla en la base de datos o si se pierde temporalmente la conexión. . A continuación, se documentan los detalles de la decisión.
...
Elección de Tácticas de Reintentos para el Módulo de Motivación. . A continuación, se documentan los detalles de la decisión.
