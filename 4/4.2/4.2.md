...
1. Simplicidad en la implementación de nuevas métricas.

2. Facilidad de modificación y mantenimiento.

3. Capacidad de reuso de la lógica de métricas en otros módulos.

4. Reducción del impacto en otras áreas del sistema.

### Decisión

Se eligió Encapsulamiento para mejorar la mantenibilidad del módulo de Seguimiento al agregar nuevas métricas.
...
| **Cod Escenario** | **Atributo de Calidad** | **Estímulo**                                                                  | **Fuente del Estímulo** | **Artefacto**                   | **Entorno**                                          | **Respuesta**                                                                              | **Medida de Respuesta**                                                                                 |
| ----------------- | ----------------------- | ----------------------------------------------------------------------------- | ----------------------- | ------------------------------- | ---------------------------------------------------- | ------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------- |
| ESC-01            | Mantenibilidad          | Modificación para la agregar funcionalidades de creación de encuestas (polls) en el módulo de mensajería | Equipo de desarrollo    | Código del Módulo de Mensajería | Operación continua con múltiples usuarios conectados | La modificación se realiza sin interrumpir el servicio y sin afectar otras funcionalidades | El tiempo requerido para implementar la creación de polls es menor a 4 horas y no se introducen errores |
...
| **Atributo de Calidad** | **Estímulo**   | **Fuente del Estímulo**                                  | **Artefacto**        | **Entorno**                      | **Respuesta**                                                      | **Medida de Respuesta**                                                                    |
| ----------------------- | -------------- | -------------------------------------------------------- | -------------------- | -------------------------------- | ------------------------------------------------------------------ | ------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------ |
| ESC-04                  | Actualizaciones en la lógica de conexión con nuevos amigos | Equipo de desarrollo | Módulo de Amigos | Entorno de desarrollo y producción | Las modificaciones se implementan de la forma más rápida | Los cambios están en producción en 2 horas |
...
2. Restringir las Dependencias: Patron Strategy
...
1. Facilitar el agregamiento de nuevas formas de agregar amigos.
2. Fomentar el mantenimiento a largo plazo.
3. Evitar errores que puedan salir al realizar actualizaciones.
...
## Cod Escenario Modulo Motivacion  
| Cod Escenario | Atributo de Calidad | Estímulo | Fuente del Estímulo | Artefacto | Entorno | Respuesta | Medida de Respuesta |
|---------------|---------------------|----------|---------------------|-----------|---------|-----------|---------------------|
| ESC-05        | Mantenibilidad       | Modificación en las reglas de generación de mensajes motivacionales personalizados | Equipo de desarrollo | Código del Módulo de Motivación | Entorno de desarrollo y producción | La modificación se realiza sin interrumpir el servicio y sin afectar la personalización de mensajes para usuarios actuales | El tiempo requerido para implementar los cambios es menor a 2 horas y no se introducen errores |
...
### Contexto  
El módulo de Motivación incluye la funcionalidad de mostrar mensajes motivacionales personalizados y enviar notificaciones a los usuarios. Con el crecimiento de la base de usuarios y los requerimientos de personalización de mensajes, es necesario que el sistema permita ajustar las reglas que generan estos mensajes sin comprometer la mantenibilidad del código. Además, se debe garantizar que las modificaciones en las reglas de generación de mensajes no afecten el rendimiento ni la disponibilidad del servicio.

### Alternativas de Tácticas  
1. **Encapsulamiento de la Lógica de Personalización**  
...
2. **Parametrización de las Reglas de Personalización**  
...
3. **Modificación Directa en el Código**  
