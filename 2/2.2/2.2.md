...
| Disponibilidad         | Usuarios activos                 | Alta demanda de usuarios recurrentes             | Módulo de Usuarios | Alta demanda                          | El sistema debe gestionar el acceso simultáneo sin retraso ni caídas                     | el sistema debe ser capaz de soportar altos volumenes de datos                      |
...
| Mantenibilidad          | Actualización de algún servicio                 | Proveedor del Servicio  | Módulo de Usuarios | El servicio correspondiente entra en modo de mantenimiento sin afectar los demás	 | No hay reclamos debido a que no hay quejas |
...
| Usabilidad | Consulta y registro de Habitos del usuraio | Usuario | Módulo de Hábitos | Operacion normal | El sistema debe devolver los resultados rapidamente | El usuario completa la operación deseada en menos de 1 minutos sin pedir ayuda |
| Usabilidad | Aprender a utilizar la interfaz de gestion de habitos | Usuario | Módulo de Hábitos | Operacion normal | El sistema debe brindar una interfaz sencilla de usar | El usuario comprende la interfaz y puede navegar y usar la aplicacion sin problemas |
...
| Escalabilidad       | Incremento de hábitos    | Consulta de datos masivos para analisis | Módulo de Analisis    | Crecimiento continuo     | El sistema debe manejar el aumento de datos sin disminuir su rendimiento        | El sistema debe poder gestionar hasta 10,000 entradas de hábitos sin ralentizaciones en el seguimiento y reporte |
| Rendimiento       | Multiples solicitudes por los usuarios    | Usuarios reportan lentitud | Módulo de Analisis    | Crecimiento continuo     | El sistema debe optimizar tiempos de respuesta para operaciones básicas.       | El sistema debe ser capaz de detectar operaciones comunes y ahorrar rendimiento de en las mismas |
| Escalabilidad       |  Usuarios   | Necesidad de migrar a nuevas regiones | Módulo de Analisis    | Crecimiento continuo     | El sistema debe ser capaz de adapatarse a nuevos entornos geograficos      | No hay quejas de usuario de diferentes regiones al usar la aplicacion |
| Seguridad       |  Usuarios   | Denegacion de ataque de DDoS | Módulo de Analisis    | operacion no frecuente    | El sistema debe ser capaz de detectar un ataque DDos y protejerse del mismo      | No hay quejas de usuario de diferentes regiones al usar la aplicacion |
